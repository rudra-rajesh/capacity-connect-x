...
(function () {
  ...
  // 2c. Click user-card (not the Logout link) opens the profile page.
  ...
  // 2b. Approvals count badge (pending notifications for admin)
  try {
    var reqs = JSON.parse(localStorage.getItem("cc_requests") || "[]");
    var count = 0;
    for (var n = 0; n < reqs.length; n++) {
      if (reqs[n].status === "pending") count++;
    }
